...
`uvm_info("TAG", "消息", UVM_LOW)
    ↓ 宏会调用 UVM 的报告机制
uvm_report_info("TAG", "消息", UVM_LOW, `__FILE__, `__LINE__);
```
...
| `` `uvm_info(tag, msg, level) `` | 报告信息 | 调用 UVM report server |
| `` `uvm_error(tag, msg) `` | 报告错误 | 计入 UVM error 计数 |
| `` `uvm_fatal(tag, msg) `` | 报告致命错误 | 终止仿真 |
...
| 宏 | 功能 | 用法示例 |
|---|---|---|
| `uvm_object_utils(T)` | 注册 uvm_object 派生类 | `uvm_object_utils(my_transaction)` |
| `uvm_object_utils_begin/end` | 注册带字段的类 | 见下方字段宏 |
| `uvm_component_utils(T)` | 注册 uvm_component 派生类 | `uvm_component_utils(my_driver)` |
| `uvm_component_param_utils(T)` | 注册带参数的组件 | `uvm_component_param_utils(my_driver #(8))` |

#### 2. 事务操作宏

| 宏 | 功能 | 关键步骤（概念化） |
|---|---|---|
| `uvm_do(req)` | 创建+随机化+发送 | create → start_item → randomize → finish_item |
| `uvm_do_with(req, {约束})` | 创建+随机化（带约束）+发送 | create → start_item → randomize with → finish_item |
| `uvm_do_on(req, seqr)` | 创建+随机化+发送到指定 sequencer | 同上，但指定 sequencer |
| `uvm_create(req)` | 只创建事务对象 | `req = type_id::create("req")` |
| `uvm_send(req)` | 只发送事务（不随机化） | start_item → finish_item |
| `uvm_send_with(req, {约束})` | 发送前按约束随机化 | start_item → randomize with → finish_item |
...
class my_sequence extends uvm_sequence #(my_transaction);
    ...
    task body();
        ...
        // 带约束发送
        `uvm_do_with(req, { addr == 32'h1000; data[7:0] == 8'hFF; })
...
| 宏 | 功能 | verbosity级别 |
|---|---|---|
| `uvm_info(tag, msg, level)` | 信息消息 | UVM_LOW/MEDIUM/HIGH/DEBUG |
| `uvm_warning(tag, msg)` | 警告消息 | - |
| `uvm_error(tag, msg)` | 错误消息（可计数） | - |
| `uvm_fatal(tag, msg)` | 致命错误（停止仿真） | - |

**使用示例**：
```systemverilog
`uvm_info("DRV", $sformatf(" Sending addr=%h", addr), UVM_LOW)
`uvm_warning("DRV", "Data width mismatch")
`uvm_error("SCB", $sformatf("Mismatch! Expected: %h, Actual: %h", exp, act))
`uvm_fatal("DRV", "Cannot create transaction")
```
...
| 宏 | 功能 |
|---|---|
| `uvm_field_int(name, flag)` | 自动化整数字段 |
| `uvm_field_object(name, flag)` | 自动化对象字段 |
| `uvm_field_enum(type, name, flag)` | 自动化枚举字段 |
| `uvm_field_array_int(name, flag)` | 自动化整数数组字段 |
| `uvm_field_string(name, flag)` | 自动化字符串字段 |

**flag选项**：UVM_ALL_ON, UVM_DEFAULT, UVM_NO_COMPARE, UVM_NO_PRINT等
...
| API | 功能 |
|---|---|
| `uvm_config_db#(T)::set(...)` | 设置配置参数 |
| `uvm_config_db#(T)::get(...)` | 获取配置参数 |
| `uvm_config_db#(T)::exists(...)` | 检查配置是否存在 |

**使用示例**：
```systemverilog
// 设置接口
uvm_config_db #(virtual my_if)::set(null, "*.drv", "vif", drv_if);

// 获取接口
uvm_config_db #(virtual my_if)::get(this, "", "vif", vif);

// 设置参数
uvm_config_db #(int)::set(null, "*.env", "num_agents", 4);
```
...
```
可读性考虑：
  - UVM 教学和多数工程通常使用 `import uvm_pkg::*;`
  - 也可只导入需要的名字，或使用 `uvm_pkg::类型名`

命名考虑：
  - 通配导入可能与其他 package 的同名标识符冲突
  - 有冲突时应使用限定名或调整 import 范围
```
...
| **有端口** | ❌ | ✅ |
| **会被综合** | ❌ | ✅ |
| **可声明 class** | ✅（常见用法） | ✅（但通常不用于组织可复用 UVM 类型） |
...
**一句话：** `import uvm_pkg::*;` 让 UVM 类名可直接使用；`uvm_macros.svh` 只在使用 UVM 宏时需要；package 用于组织代码，但不是使用 UVM 的硬性前提。
...
class adpcm_driver extends uvm_driver;
    virtual adpcm_if.drv_mp ADPCM;  // 虚拟接口句柄

    function void build_phase(uvm_phase phase);
        // 从 config_db 获取实际接口
        if (!uvm_config_db #(virtual adpcm_if.drv_mp)::get(this, "", "ADPCM_vif", ADPCM))
            `uvm_fatal("NOVIF", "adpcm_driver 未取得 ADPCM_vif")
    endfunction

    task run_phase(uvm_phase phase);
        @(ADPCM.drv_cb);  // 通过 vif 访问硬件信号
        ADPCM.drv_cb.frame <= 1;
        ADPCM.drv_cb.data <= 4'hA;
    endtask
endclass
```
...
module testbench;
    ...
    initial begin
        ...
        uvm_config_db #(virtual adpcm_if.drv_mp)::set(
            null, "*", "ADPCM_vif", adpcm_if_inst.drv_mp
        );
        run_test();
    end
endmodule
...
class adpcm_driver extends uvm_driver #(adpcm_seq_item);
    ...
    virtual adpcm_if.drv_mp ADPCM;         // 接口句柄
    ...
    task run_phase(uvm_phase phase);
        ...
        ADPCM.drv_cb.frame <= 0;          // 初始值
        ADPCM.drv_cb.data <= 0;
        // 这个 forever 循环：发送数据
        forever begin
            seq_item_port.get_next_item(req);  // 1. 从 Sequencer 拿数据

            repeat(req.delay) @(ADPCM.drv_cb); // 2. 等待 delay 个周期
            shift_data = req.data;             // 不修改 sequence item 本身

            ADPCM.drv_cb.frame <= 1;           // 3. 帧开始
            for(int i = 0; i < 8; i++) begin   // 4. 发送 8 个 4-bit
                @(ADPCM.drv_cb);
                ADPCM.drv_cb.data <= shift_data[3:0]; // 发低 4 位
                shift_data = shift_data >> 4;
            end
            ADPCM.drv_cb.frame <= 0;           // 5. 帧结束
...
| `@(ADPCM.drv_cb)` | 等待 clocking block 的采样事件 |
...
uvm_config_db #(类型)::set(
    null,              // context：null 表示从 uvm_root 开始
    "*",               // inst_name：相对 context 的目标组件路径通配符
    "键名",            // field_name：配置字段名
    值                 // 值（纸条上的内容）
);
...
uvm_config_db #(类型)::get(
    this,              // context：从当前 component 开始查找
    "",                // inst_name：空串表示当前 component 自身
    "键名",            // field_name：配置字段名
    变量               // 接收值
);
```
...
top_tb 设置：  set(null, "*", "ADPCM_vif", vif)
driver 获取：  get(this, "", "ADPCM_vif", vif)
```
...
```
config_db 更像“按层级路径投递的配置”：

set = 为指定组件路径写入配置
get = 从当前组件的层次上下文查找匹配配置

`*` 虽可广泛匹配，但会降低可追踪性；工程中优先使用明确的实例路径。
同一字段存在多个匹配项时，选择受层次与设置顺序影响，应避免依赖含糊的通配规则。
```
...
class my_scoreboard extends uvm_scoreboard;
    ...
    uvm_analysis_imp #(my_seq_item, my_scoreboard) actual_imp;
    ...
    // 参考模型/预测器调用此函数写入预期事务
    function void push_expected(my_seq_item expected);
        expected_queue.push_back(expected);
    endfunction

    // monitor 的 analysis_port 连接到 actual_imp 后调用此函数
    function void write(my_seq_item actual);
        my_seq_item expected;
        if (expected_queue.size() == 0) begin
            `uvm_error("SCOREBOARD", "收到实际事务，但没有对应预期事务")
            return;
        end
        expected = expected_queue.pop_front();
        if (!actual.compare(expected)) begin
            `uvm_error("SCOREBOARD", $sformatf("Mismatch! exp=%s act=%s",
    ...
    function void check_phase(uvm_phase phase);
        if (expected_queue.size() != 0)
            `uvm_error("SCOREBOARD", $sformatf("仍有 %0d 个预期事务未收到", expected_queue.size()))
    endfunction
endclass
```
...
// 连接（通常在 env 的 connect_phase；也可由 agent 按架构负责）
...
class my_coverage extends uvm_component;
    ...
    my_seq_item tr;
    uvm_analysis_imp #(my_seq_item, my_coverage) analysis_imp;
    ...
    // monitor 的 analysis_port 连接到 analysis_imp 后采样
    function void write(my_seq_item item);
        tr = item;
        cg.sample();
    endfunction
...
class my_seq_item extends uvm_sequence_item;
    ...
    function void do_copy(uvm_object rhs);
        ...
        if (!$cast(rhs_, rhs)) begin
            `uvm_fatal("COPY", "my_seq_item 类型转换失败")
            return;
        end
        data = rhs_.data;
        delay = rhs_.delay;
...
class virtual_seq extends uvm_sequence;
    ...
    task body();
        seq_a_inst = seq_a::type_id::create("seq_a_inst");
        seq_b_inst = seq_b::type_id::create("seq_b_inst");
        if (seqr_a == null || seqr_b == null)
            `uvm_fatal("NOVSEQ", "virtual sequence 未配置子 sequencer")
        // 同时启动两个 Sequence
        fork
...
// RAL 访问通常在 sequence 中完成；map 已在环境中绑定 bus sequencer 与 adapter
class reg_access_seq extends uvm_sequence;
    ...
    task body();
        ...
        ctrl.write(status, 32'hFF, UVM_FRONTDOOR);
        if (status != UVM_IS_OK)
            `uvm_error("RAL", "ctrl 写失败")

        status_reg.read(status, data, UVM_FRONTDOOR);
        if (status != UVM_IS_OK)
            `uvm_error("RAL", "status 读失败")
    endtask
endclass
```
