...
<img src="/AIInfraGuide/images/dp_diagram.png" alt="数据并行：模型复制到多卡，各卡在不同数据微批上并行前反向，再同步梯度" style="max-width: 85%; display: block; margin: 0 auto;" />
...
<img src="/AIInfraGuide/images/dp_overlap1.svg" alt="朴素 DP：先完成整个反向传播，再统一同步梯度，通信期间 GPU 空闲" style="max-width: 100%; display: block; margin: 0 auto;" />
...
<img src="/AIInfraGuide/images/dp_overlap2.svg" alt="梯度同步与反向传播重叠：靠后层的梯度先就绪即可立即同步，与更靠前层的反向计算并行" style="max-width: 100%; display: block; margin: 0 auto;" />
...
<img src="/AIInfraGuide/images/dp_overlap3.svg" alt="梯度分桶：把多个梯度打包成一个 Bucket，只发起一次 AllReduce，减少通信次数" style="max-width: 100%; display: block; margin: 0 auto;" />
...
<img src="/AIInfraGuide/images/dp_scaling.svg" alt="随 DP rank 增加的吞吐量与单卡显存占用：超过某上限后吞吐量下降，单卡显存恒定" style="max-width: 100%; display: block; margin: 0 auto;" />
